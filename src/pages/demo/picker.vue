<template>
  <div>
    <div> {{ 'Normal usage, the first one is selected without setting default value' }} {{year1}}</div>
    <picker :data='years' v-model='year1' @on-change='change'></picker>
    <br>
    <div> {{ 'Asynchronous loading and dynamic change of data' }} </div>
    <picker :data='years001' v-model='year001' @on-change='change'></picker>
    <br>
    <cell-group>
      <cell :title="'Current value'" :value="year001"></cell>
    </cell-group>
    <button-cell type="primary" @click.native="changeValue([['1','3','5','7','9','11'],['2','3','4','5'],['a','b','c']])"> {{ 'Set Data 1' }} </button-cell>
    <button-cell type="primary" @click.native="changeValue([['1','3','5','7','9','11'],['2','3','4','5']])">{{ 'Set Data 2' }} </button-cell>
    <button-cell type="primary" @click.native="changeValue([['2','4','6','8','10','11']])">{{ 'Set Data 3' }} </button-cell>
    <br>
    ...
    <br>
    <div> {{ 'Multi cols in non chained-mode' }} </div>
    ...
    <br>
    <div>{{ 'Locations in chained-mode' }}: {{ 'Current value' }} {{year7Value}} <br> {{ 'Text corresponding to the value' }}: {{$refs.picker1&&$refs.picker1.getNameValues()}}</div>
    <picker :data='year7' :columns=3 v-model='year7Value' @on-change='change' ref="picker1"></picker>
    <button-cell @click.native="setData1" type="primary"> {{ 'Set value to' }} ["USA", "usa002", "0005"]</button-cell>
    <button-cell @click.native="setData2" type="primary"> {{ 'Set value to' }} ["china", "china002", "gz"]</button-cell>
    <button-cell @click.native="setList" type="primary">Set List</button-cell>
    <br>
    <div> {{ '3 cols data but only show 2 cols' }} </div>
