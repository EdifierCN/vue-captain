<template>
  <a
    class="zmui-datetime zmui-cell"
    :class="{'zmui-cell_access': !readonly}"
    data-cancel-text="取消"
    ...
    <slot>
      ...
      <div class="zmui-cell__ft zmui-cell-primary zmui-datetime-value" :style="{textAlign: valueTextAlign}">
        <span class="zmui-cell-placeholder" v-if="!currentValue && placeholder">{{ placeholder }}</span>
        <span class="zmui-cell-value" v-if="currentValue">{{ displayFormat ? displayFormat(currentValue) : currentValue }}</span>
        <icon class="zmui-input-icon" type="warn" v-show="!valid" :title="firstError"></icon>
      </div>
    </slot>
  </a>
</template>
  ...
  export default {
    ...
    props: {
      ...
      endDate: {
        ...
        validator (val) {
          if (process.env.NODE_ENV === 'development' && val && val.length !== 10) {
            console.error('Datetime prop:end-date 必须为 YYYY-MM-DD 格式')
          }
          return val ? val.length === 10 : true
        }
      },
    ...
    mounted () {
      ...
      this.$el.setAttribute('id', `zmui-datetime-${uuid}`);
      if (!this.readonly) {
        this.$nextTick(() => {
          this.render();
    ...
    computed: {
      pickerOptions () {
        const _this = this;
        const options = {
          trigger: '#zmui-datetime-' + this.uuid,
          format: this.format,
          value: this.currentValue,
          output: '.zmui-datetime-value',
...
<style lang="less">
  @import "../../../styles/widget/datetime/index";
</style>
