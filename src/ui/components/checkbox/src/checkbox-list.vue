<template>
  <div :class="disabled ? 'zmui-checklist-disabled' : ''">
    <div v-show="title" class="zmui-cells__title">{{ title }}</div>
    <slot name="after-title"></slot>
    <div class="zmui-cells zmui-cells_checkbox">
      <label class="zmui-cell zmui-check_label" :class="{'zmui-checklist-label-left': labelPosition === 'left'}" :for="`checkbox_${uuid}_${index}`" v-for="(one, index) in currentOptions">
        <div class="zmui-cell__hd">
          <input type="checkbox" class="zmui-check" :name="`zmui-checkbox-${uuid}`" :value="getKey(one)" v-model="currentValue" :id="disabled ? '' : `checkbox_${uuid}_${index}`" :disabled="isDisabled(getKey(one))">
          <i class="zmui-icon-checked zmui-checklist-icon-checked"></i>
        </div>
        <div class="zmui-cell__bd">
          <p v-html="getValue(one)"></p>
          <span class="zmui-cell-label-desc" v-if="getInlineDesc(one)">T{{ getInlineDesc(one) }}</span>
        </div>
      </label>
    </div>
...
<style lang="less">
  @import '../../../styles/widget/checkbox/checkbox-list';
</style>
