...
const Store = class {
  ...
  genDates (year, month) {
    ...
    if (current.clone().startOf('month').day() !== 0) {
      let start = current.clone().startOf('month').day();  // sunday is 0, change to 7 to make it easy to caculate
    ...
    for (i = current.clone().endOf('month').day(); ; i++) {
      add({
        date: next.clone(),
        nextMonth: true
      });
      if (next.day() === 6 && dates.length >= 42) {
        break
      }
      next.add(1, 'days')
    }
