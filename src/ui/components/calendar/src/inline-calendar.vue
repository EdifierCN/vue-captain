<template>
  <div class="inline-calendar" :class="{'is-weekend-highlight': highlightWeekend}">
    ...
    <div class="calendar-header" v-show="!hideHeader">
      <div class="calendar-year">
        <span @click="go(year - 1, month)">
          <a class="year-prev zmui-prev-icon" href="javascript:"></a>
        </span>
        ...
        <span class="calendar-header-right-arrow" @click="go(year + 1, month)">
          <a class="year-next zmui-next-icon" href="javascript:"></a>
        </span>
      </div>

      <div class="calendar-month">
        <span @click="prev">
          <a class="month-prev zmui-prev-icon" href="javascript:"></a>
        </span>
        ...
        <span @click="next" class="calendar-header-right-arrow">
          <a class="month-next zmui-next-icon" href="javascript:"></a>
        </span>
      </div>
    </div>
    ...
    <table>
      ...
      <tr>
        <th v-for="(week, index) in _weeksList" class="week" :class="`is-week-list-${index}`">{{ week || ('星期' + index) }}</th>
      </tr>
      </thead>
      <tbody>
      <tr v-for="(day,k1) in days">
        <td
          ...
          <slot
            ...
            class-name="zmui-calendar-each-date"
            :row="k1"
            :col="k2"
            ...
            name="each-day">
              <span
                class="zmui-calendar-each-date"
                :style="getMarkStyle(child)"
                v-show="showChild(year, month, child)">
                {{ replaceText(child.day, formatDate(year, month, child)) }}
                <span class="zmui-calendar-top-tip" v-if="isShowTopTip(child)" :style="isShowTopTip(child, 'style')">
                  <span>{{ isShowTopTip(child, 'text') }}</span>
                </span>
              </span>
            <span class="zmui-calendar-dot" v-if="isShowBottomDot(child)"></span>
...
<script>
  import format from '../../datetime/src/format'
...
<style lang="less">
  ...
  .inline-calendar td > span.zmui-calendar-each-date {
    position: relative;
    display: inline-block;
  ...
  .inline-calendar td.current > span.zmui-calendar-each-date {
