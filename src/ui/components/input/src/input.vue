<template>
  <div class="zmui-input-cell zmui-cell" :class="{'zmui-cell_warn': showWarn, 'disabled': disabled}">
    <div class="zmui-cell__hd">
      <div :style="labelStyles" v-if="hasRestrictedLabel">
        <slot name="restricted-label"></slot>
      </div>
      <slot name="label">
        <label class="zmui-cell-label" :class="labelClass" :style="{width: labelWidth || $parent.labelWidth || labelWidthComputed, textAlign: $parent.labelAlign, marginRight: $parent.labelMarginRight}" v-if="title" v-html="title" :for="`zmui-input-${uuid}`"></label>
        <span class="zmui-cell-label-desc" v-if="inlineDesc">{{ inlineDesc }}</span>
      </slot>
    </div>
    <div class="zmui-cell__bd zmui-cell__primary" :class="placeholderAlign ? `zmui-input-placeholder-${placeholderAlign}` : ''">
      <input
        :id="`zmui-input-${uuid}`"
        v-if="!type || type === 'text'"
        class="zmui-input"
    ...
    <div class="zmui-cell__ft">
      ...
      <icon @click.native="onClickErrorIcon" class="zmui-input-icon" type="warn" :title="!valid ? firstError : ''" v-show="showWarn"></icon>
      <icon @click.native="onClickErrorIcon" class="zmui-input-icon" type="warn" v-if="!novalidate && hasLengthEqual && dirty && equalWith && !valid"></icon>

      <icon type="success" v-show="!novalidate && equalWith && equalWith === currentValue && valid"></icon>

      <icon type="success" class="zmui-input-icon" v-show="novalidate && iconType === 'success'"></icon>
      <icon type="warn" class="zmui-input-icon" v-show="novalidate && iconType === 'error'"></icon>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      labelClass () {
        return {
          'zmui-cell-justify': this.$parent.labelAlign === 'justify' || this.$parent.$parent.labelAlign === 'justify'
        }
      },
      pattern () {
...
<style lang="less">
  ...
  @import '../../../styles/widget/cell/form/vcode';

  .zmui-input .zmui-input-placeholder-right input::-webkit-input-placeholder {
    text-align: right;
  }
  .zmui-input .zmui-input-placeholder-center input::-webkit-input-placeholder {
    text-align: center;
  }
  .zmui-input .zmui-input-icon {
    font-size: 21px;
  }
  .zmui-input-icon.zmui-icon-warn:before, .zmui-input-icon.zmui-icon-success:before {
    font-size: 21px;
  }
  .zmui-input .zmui-icon {
    padding-left: 5px;
  }
  .zmui-input.zmui-cell_vcode {
    padding-top: 0;
    padding-right: 0;
    padding-bottom: 0;
  }
