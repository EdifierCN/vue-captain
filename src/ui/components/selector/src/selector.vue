<template>
  <div class="zmui-selector zmui-cell" :class="{'zmui-cell_select':!readonly, 'zmui-cell_select-after':title}">
    <div class="zmui-cell__hd" v-if="title">
      <label
        :for="`zmui-selector-${uuid}`"
        class="zmui-label"
        :class="labelClass"
        :style="cleanStyle({
    ...
    <div class="zmui-cell__bd" v-if="!readonly">
      <select :id="`zmui-selector-${uuid}`" style="color:red;" class="zmui-select" v-model="currentValue" :name="name"
              :style="cleanStyle({
        direction: direction,
        color: color
      })">
    ...
    <div class="zmui-cell__ft zmui-selector-readonly" v-else>
        ...
        validator (val) {
          if (!val.length || val.length !== 2) {
            if (process.env.NODE_ENV === 'development') {
              console.error('[zmui error] selector prop:valueMap\'s length should be 2')
            }
            return false
          }
          return true
...
<style lang="less">
  ...
  @import '../../../styles/widget/cell/form/select';

  .zmui-selector-no-padding {
    padding-left: 0;
  }
  .zmui-selector.zmui-cell_select {
    padding: 0;
  }
  .zmui-selector.zmui-cell_select-after {
    padding-left: 15px;
  }
  .zmui-selector-readonly {
    width: 100%;
  }
</style>
