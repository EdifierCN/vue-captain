<template>
  <div class="zmui-emotion"><slot style="display:none;"></slot></div>
</template>
...
export default {
  ...
  mounted () {
    ...
    const name = this.$el.innerHTML.replace(/(^\s*)|(\s*$)/g, "");
    this.index = list.indexOf(name);
    this.backgroundPositionX = -this.index * 24;
    let imgHTML = '';

    if (!this.isGif) {
      imgHTML = `<span class="zmui-static-emotion" style="background:url(https://res.wx.qq.com/mpres/htmledition/images/icon/emotion/default218877.gif) ${this.backgroundPositionX}px 0;"></span>`
    } else {
      imgHTML = `<img src="https://res.wx.qq.com/mpres/htmledition/images/icon/emotion/${this.index}.gif" style="width: 30px; height: 30px;">`
    }
    this.$nextTick(() => {
  ...
  computed:{
    emotionGroup(){
      const ems = this.emotions;
      const arr = [];
      for(let k = 0,i = 0,g = [], len = ems.length; i<len; i++){
        if(i+1 >= (k===0 ? 22 : 22 * (k+1))){
          k++;
          arr.push(g);
          g = [];
        }else{
