<template>
  <div class="zmui-cell">
    ...
    <div class="zmui-cell__ft zmui-cell-primary" :class="{'zmui-number-round': buttonStyle === 'round'}" v-show="!readonly" style="font-size:0">
      <div :style="{float:align}">
        <a @click="sub" class="zmui-number-selector zmui-number-selector-sub":class="{'zmui-number-disabled':disabledMin}">
        ...
        <input v-model.number="currentValue" :name="name" class="zmui-number-input" :style="{width: width}" :readonly="!fillable" pattern="[0-9]*" type="number" @blur="blur" />
        <a @click="add" class="zmui-number-selector zmui-number-selector-plus" :class="{'zmui-number-disabled':disabledMax}">
    ...
    <div class="zmui-cell__ft zmui-cell-primary" v-show="readonly">
      {{value}}
    </div>
  </div>
</template>
...
<style lang="less">
  ...
  .zmui-number-round .zmui-number-input {
    border: none;
  }
  .zmui-number-selector {
    float:left;
    height:20px;
    font-size:25px;
    line-height:18px;
  ...
  .zmui-number-round .zmui-number-selector {
    width: 20px;
    border-radius: 13px;
  }
  .zmui-number-selector svg {
    fill: @number-button-font-color;
  }
  .zmui-number-selector.zmui-number-disabled svg {
    fill: #ccc;
  }
  .zmui-number-round .zmui-number-selector.zmui-number-disabled {
    border-color: @number-round-button-disabled-border-color;
  }
  .zmui-number-round .zmui-number-selector.zmui-number-disabled svg {
    fill: #ccc;
  }
  .zmui-number-selector-sub {
    border-right:none;
    padding:4px 8px 2px 8px;
    border-radius:2px 0 0 2px;
  }
  .zmui-number-selector-plus {
    border-left:none;
    margin-right: 5px;
    padding:3px 8px;
    border-radius:0 2px 2px 0;
  }
  .zmui-number-round .zmui-number-selector-sub svg {
    position: relative;
    top: 1px;
  }
  .zmui-number-round .zmui-number-selector-sub, .zmui-number-round .zmui-number-selector-plus {
    padding: 2px;
