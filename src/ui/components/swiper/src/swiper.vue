<template>
  <div class="zmui-swiper-container">
    <div class="zmui-swiper" :style="{height: xheight}">

      <slot></slot>

      <div
      class="zmui-swiper-item"
      v-if="!hasSliderSlot"
      v-for="(item, index) in list"
      @click="clickListItem(item)"
      :data-index="index">
        <a href="javascript:">
          <div class="zmui-swiper-img" :style="{backgroundImage: buildBackgroundUrl(item)}"></div>
          <p class="zmui-swiper-desc" v-if="showDescMask">{{ item.title }}</p>
        </a>
      </div>

      <div
      v-if="listTwoLoopItem.length > 0"
      class="zmui-swiper-item zmui-swiper-item-clone"
    ...
    <div :class="[dotsClass, 'zmui-indicator', `zmui-indicator-${dotsPosition}`]" v-show="showDots">
      <a href="javascript:" v-for="key in length">
        <i class="zmui-icon-dot" :class="{'active': key - 1 === current}"></i>
      </a>
    </div>
  </div>
</template>
...
export default {
  name: 'swiper',
  created () {
    this.hasSliderSlot = !!this.$slots.default;
  ...
  methods: {
    ...
    getHeight () {
      ...
      if (!hasHeight) {
        if (this.aspectRatio) {
          return this.getDomSize('width',this.$el) * this.aspectRatio + 'px'
        }
        return '180px'
      }
    },
    getDomSize(key, obj){
      ...
      obj.parentNode.parentNode.appendChild(clone);

      let width = clone.offsetWidth;
      let height = clone.offsetHeight;

      obj.parentNode.parentNode.removeChild(clone);
  ...
  watch: {
    ...
    index (val) {
      ...
      if (val !== this.current) {
        this.$nextTick(() => {
          _this.swiper && _this.swiper.go(val, _this.navModel ? 1 : null)
        })
      }
      this.$emit('input', val)
    },
